...
- [ ] 🔴 **开发环境配置**
  - [ ] 配置Go 1.21+ 开发环境（后端）
  - [ ] 配置Node.js 18+ LTS开发环境（前端）
  - [ ] 配置pnpm 8.x 包管理工具（monorepo支持更好）
  - [ ] 配置ESLint + Prettier代码规范（前端）
    - ESLint规则：@typescript-eslint/recommended
  - [ ] 配置golangci-lint代码规范（后端）
...
- [ ] 🔴 **后端项目初始化**
  ...
  - [ ] 配置Viper环境变量管理
    ...
    - 环境变量覆盖
    - 多环境配置
  - [ ] 配置Swagger API文档（swaggo/echo-swagger）
    - API分组（按模块）
    - 请求/响应示例
    - 认证配置
  - [ ] 配置全局异常处理中间件（Echo内置）
  - [ ] 配置请求验证（Echo内置Validator + go-playground/validator）
  - [ ] 配置响应统一格式中间件
  - [ ] 配置跨域中间件（echo.middleware.CORS）
  - [ ] 配置安全头中间件（echo.middleware.Secure）
  - [ ] 配置速率限制中间件（echo.middleware.RateLimiter）

- [ ] 🔴 **移动端项目初始化**
  - [ ] 创建Taro项目（React + TypeScript）
    ```bash
...
- [ ] 🔴 **Ant Design 环境配置**
  ...
  - [ ] 配置国际化
    ...
    - 配置日期组件locale
  - [ ] 配置按需加载（优化打包体积）
  - [ ] 创建全局样式文件（assets/css/antd-custom.css）

- [ ] 🔴 **Ant Design 组件使用规范**
  - [ ] 所有UI组件必须使用Ant Design提供的组件
    - Button（按钮）
    - Input（输入框）
    - Select（选择器）
    - Table（表格）
    - Modal（对话框）
    - Form（表单）
  - [ ] 组件样式定制通过ConfigProvider和自定义CSS实现
  - [ ] 禁止使用其他UI框架的组件，避免样式冲突
  - [ ] 特殊业务组件基于Ant Design组件进行封装扩展
...
- [ ] 🔴 **接口安全**
  ...
  - [ ] 实现接口频率限制（使用Echo RateLimiter中间件）
    - 普通接口：100次/分钟
    - 登录接口：10次/分钟
    - 验证码接口：1次/分钟
  - [ ] 实现SQL注入防护
...
- [ ] 🔴 **供应商管理模块API**
  ...
  - [ ] 启用/禁用供应商
  - [ ] 配送区域管理接口
  - [ ] Model、Service、Handler、Router文件
...
- [ ] **加价计算逻辑实现**
  ...
  - [ ] 查找匹配的加价规则
    - 优先级从高到低：
      1. 门店+供应商+商品 精确匹配（优先级最高）
      2. 门店+供应商 / 门店+商品 / 供应商+商品
      3. 仅门店 / 仅供应商 / 仅商品
  - [ ] 取优先级最高的已启用规则
    - 如果加价值为0，表示不加价
    - 如果无匹配规则，不加价
...
- [ ] **推送服务**
  - [ ] Webhook推送服务类（WebhookService）
  - [ ] 消息格式化处理（根据事件类型和目标类型格式化）
  - [ ] Markdown格式消息构建
  - [ ] HTTP请求发送（POST请求到Webhook URL）
  - [ ] 响应结果处理
  - [ ] 推送日志记录
  - [ ] 多群推送支持（一个订单推送到多个群）
  - [ ] 推送任务队列（使用Redis队列）
...
- [ ] **失败重试机制**
  ...
  - [ ] 自动重试（最多3次，间隔5分钟）
  - [ ] 重试次数记录
  - [ ] 指数退避策略（第1次5分钟，第2次15分钟，第3次30分钟）
  - [ ] 连续失败告警（推送到管理员群）
  - [ ] 定时任务扫描重试队列

- [ ] **推送日志**
  - [ ] 推送日志记录（WebhookLog表）
  - [ ] 日志查询接口（GET /admin/webhook-logs）
  - [ ] 日志清理策略（保留30天）
  - [ ] 推送统计接口（成功率、平均耗时等）
...
- [ ] **推送服务**
  ...
  - [ ] 订单恢复事件推送
  - [ ] 请求签名生成（HMAC-SHA256）
  - [ ] 请求头设置（Content-Type, X-Signature, X-Timestamp, X-Nonce）
  - [ ] 异步推送队列（避免阻塞主流程）
  - [ ] 推送结果记录
...
- [ ] **配置管理接口**
  - [ ] POST /admin/suppliers/{id}/api-config - 配置API对接
  - [ ] PUT /admin/suppliers/{id}/api-config - 更新API配置
  - [ ] POST /admin/suppliers/{id}/regenerate-key - 重新生成密钥
  - [ ] POST /admin/suppliers/{id}/test-push - 测试推送
